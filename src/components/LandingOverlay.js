...
const LandingOverlay = ({ clickCaret, caretOpts, options, isMobile }) => {
  const { showcase } = useContext(showcaseContext)
  const ref = useRef(null)

  let FadingButtons = WithFade(
    isMobile ? ActionButton : ActionButtons,
    2,
    1,
  ...
  return (
    <div className="landing-overlay">
      ...
      <Toolbar hide={caretOpts.up} options={options} />
    </div>
  )
}
